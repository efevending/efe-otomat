...
router.get('/', (req: Request, res: Response) => {
  ...
  let query = `
    SELECT w.*, u.full_name as manager_name, s.name as supplier_name,
      ...
      v.plate as vehicle_plate, v.driver_name as vehicle_driver_name
    FROM warehouses w
...
router.post('/', requireRole('admin'), (req: Request, res: Response) => {
  const { name, type, address, manager_id, supplier_id, special_code, responsible_user_id, vehicle_id } = req.body;
  if (!name || !type) return res.status(400).json({ error: 'Depo adı ve tipi gerekli' });

  const result = db.prepare('INSERT INTO warehouses (name, type, address, manager_id, supplier_id, special_code, responsible_user_id, vehicle_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)')
    .run(name, type, address || null, manager_id || null, supplier_id || null, special_code || '', responsible_user_id || null, vehicle_id || null);
...
router.put('/:id', requireRole('admin'), (req: Request, res: Response) => {
  ...
  db.prepare('UPDATE warehouses SET name = ?, type = ?, address = ?, manager_id = ?, supplier_id = ?, special_code = ?, responsible_user_id = ?, vehicle_id = ? WHERE id = ?')
    .run(name ?? warehouse.name, type ?? warehouse.type, address ?? warehouse.address,
      ...
      vehicle_id ?? warehouse.vehicle_id,
      req.params.id);
